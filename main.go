...
func generateKeyPair(filename string) error {
	...
	addr, err := bitumutil.NewAddressPubKeyHash(
		bitumutil.Hash160(pub.SerializeCompressed()),
		&params,
	...
	privWif, err := bitumutil.NewWIF(priv, &params, bitumec.STEcdsaSecp256k1)
	if err != nil {
		return err
	}
...
func main() {
	...
	helpMessage := func() {
		fmt.Println(
			"Usage: bitumaddrgen [-testnet] [-simnet] [-noseed] [-verify] [-h] filename")
		fmt.Println("Generate a Bitum private and public key or wallet seed. \n" +
